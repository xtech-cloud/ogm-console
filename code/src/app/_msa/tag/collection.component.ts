...
export class TagCollectionComponent implements OnInit{
  ...
  onSearchClick() : void {
      this.tagService.collectionList(this.search.offset, this.search.count,  (_status, _entity, _total)=>{
          if(0 != _status.code) {
          ...
          for (var i = 0; i < _entity.length; i++) {
              let tag = {
                  ...
                  name: _entity[i]['name'],
                  flag: _entity[i]['flag'],
                  alias: _entity[i]['alias']['en_US'],
                  keyword: _entity[i]['keyword'],
              };
              this.tags.push(tag);
          }
          this.source = new LocalDataSource(this.tags); 
      });
  }
