...
import * as tableData from './workflow.data';
...
export class ApprovalWorkflowComponent implements OnInit{
  ...
  modalMessage:string = '';
  WorkflowMode = [ 'Invalid', 'Parallel', 'Queue'];
  modal = {
      name: '',
      mode: 'Parallel',
  };
  ...
  onSearchClick() : void {
      this.approvalService.workflowList(this.search.offset, this.search.count,  (_status, _entity, _total)=>{
          if(0 != _status.code) {
  ...
  submitNew() : void {
      let mode = this.WorkflowMode.indexOf(this.modal.mode);
      this.approvalService.workflowMake(
          this.modal.name, 
          mode,
          (_status)=>{
           if(0 == _status.code) {
